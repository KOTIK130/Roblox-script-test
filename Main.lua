...
return RayfieldLibrary
        ...
        -- [[ ВСТАВИТЬ ЭТО В КОНЕЦ Main.lua ПОСЛЕ КОДА БИБЛИОТЕКИ ]] --

-- Получаем библиотеку (она загрузилась выше)
local Rayfield = RayfieldLibrary -- Rayfield обычно создает глобальную переменную или возвращает таблицу

if not Rayfield then
    -- Если переменная не создалась, попробуем загрузить по ссылке (резерв)
   local success, lib = pcall(function()
       return loadstring(game:HttpGet('https://raw.githubusercontent.com/SiriusSoftwareLtd/Rayfield/main/source.lua'))()
   end)
   if success then Rayfield = lib end
end
...
Import("Tabs/Players.lua")(Context)
